...
class GeoJson(Layer):
    ...
    marker: Marker, CircleMarker, Circle currently.
        If GeoJson geometry for a feature is a Point, choose a folium  class to
        represent it, with location=None.
    ...
    _template = Template(u"""
        ...
        {%- if this.marker %}
        let {{ this.get_name() }}pointToLayer = function (feature, latlng) {
        ...
        {%- if this.highlight %}
            {{this.get_name()}}_onEachFeature = function onEachFeature(feature, layer) {
                ...
                layer.on({
                    mouseout: function(e) {
                        let style = e.target.feature.properties.style
                        isMarker?e.target.setIcon(new L.{% if this.marker.icon_type %}{{this.marker.icon_type}}{% else %}Icon{% endif %}(style)):e.target.setStyle(style)},
                    mouseover: function(e) {
                        let highlight = e.target.feature.properties.highlight
                        isMarker?e.target.setIcon(new L.{% if this.marker.icon_type %}{{this.marker.icon_type}}{% else %}Icon{% endif %}(highlight)):e.target.setStyle(highlight)},
                    mouseenter: function(e) {
                        let highlight = e.target.feature.properties.highlight
                        isMarker?e.target.setIcon(new L.{% if this.marker.icon_type %}{{this.marker.icon_type}}{% else %}Icon{% endif %}(highlight)):e.target.setStyle(highlight)},
                    click: function(e) {
                        let feat = e.target.feature
                        if(feat.geometry.type!='Point'){
                        {{this._parent.get_name()}}.fitBounds(e.target.getBounds())
                        } else {{this._parent.get_name() }}.setView(e.target.getLatLng(),
                        Object.keys(feat.properties.highlight).includes('zoom')?feat.properties.highlight.zoom:10)
                        e.target.bringToFront()}
                    });
            };
        {%- endif %}
        var {{this.get_name()}} = L.geoJson(
            // start of jtbaker gejsonmarker branch
...
class DivIcon(MacroElement):
    ...
    def __init__(self, html=None, icon_size=None, icon_anchor=None,
                 popup_anchor=None, className='empty'):
        ...
        self.options = json.dumps({
            camelize(attr): getattr(self, attr) for attr in
            ['icon_size', 'icon_anchor', 'popup_anchor', 'html', 'className']
        })
...
class CustomIcon(Icon):
    ...
    def __init__(self, icon_image, icon_size=None, icon_anchor=None,
        ...
        self.options = json.dumps({
            camelize(attr): getattr(self, attr) for attr in
            ['icon_url', 'icon_size', 'icon_anchor', 'shadow_url',
             'shadow_size', 'shadow_anchor', 'popup_anchor']
        })
